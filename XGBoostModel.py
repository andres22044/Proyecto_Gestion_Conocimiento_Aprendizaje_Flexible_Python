...
class XGBoostTPUPropertyPredictor:
    ...
    def predict(self, hsp, healing_time, peak_logm, molecular_weight, 
        ...
        # --- PASO 1: Imprimir entradas ---
        print("\n" + "="*70)
        print("PREDICCIÓN DE PROPIEDADES (XGBoost)")
        print(f"  • HSP: {hsp}, Healing Time: {healing_time} hrs")
        ...
        # --- PASO 3: Normalizar y Predecir ---
        input_scaled = self.scaler.transform(input_data)
        prediction = self.model.predict(input_scaled)
        
        # --- PASO 4: Formatear como DICCIONARIO ---
        results_raw = dict(zip(self.target_names, prediction[0]))
        
        # --- PASO 5: ¡SOLUCIÓN AL ERROR JSON! ---
        # Convertir float32 de NumPy a float nativo de Python
        results_serializable = {} # <-- ¡NUEVO!
        for key, value in results_raw.items(): # <-- ¡NUEVO!
            results_serializable[key] = float(value) # <-- ¡NUEVO!
        # --- FIN DE LA SOLUCIÓN ---
        
        # --- PASO 6: Imprimir el diccionario ---
        print("\n" + "="*70)
        print("PROPIEDADES PREDICHAS:")
        print("="*70)
        # Usamos el nuevo diccionario 'results_serializable'
        for key, value in results_serializable.items(): # <-- ¡MODIFICADO!
